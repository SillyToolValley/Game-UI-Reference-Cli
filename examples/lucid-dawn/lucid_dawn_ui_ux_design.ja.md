...
| バージョン | 日付 | 変更 |
| --- | --- | --- |
| v1.0 (EN master) | 2026-06-30 | アウトゲーム(O1–O6)+インゲーム(I1–I6) の注釈付きワイヤーフレームデザイン全編、Web 収集の参照（interfaceingame、8 ゲーム）、デザイントークン + 意思決定トラッカー、任意のアートコンセプトパス、エンジンバインディング（App. D）、ユーザビリティテスト計画（App. E）。13 枚のワイヤーフレームが lint+render を通過。このマスターから韓国語/中国語版を派生。 |
...
I3 レベルアップ / アイテム選択画面の UI アート方向性コンセプトを 1 枚含める:

![レベルアップ UI アートコンセプト](art-concepts/levelup-ui-art-concept.png)

ソースプロンプト: [`art-concepts/levelup-ui-art-concept.prompt.md`](art-concepts/levelup-ui-art-concept.prompt.md)。
この画像は I3 ワイヤーフレーム、GDD/デザイン文脈、ビジュアルトークン、収集済みリファレンスメモを基に生成した。
ムード/素材感の方向確認用であり、正確な文言・寸法・状態・データバインディング・アクセシビリティは本デザインドキュメントとトークンファイルを基準にする。
